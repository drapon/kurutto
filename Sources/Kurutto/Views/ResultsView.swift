    ...
    private var progressChartSection: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            if !statsViewModel.weeklyProgress.isEmpty {
                Chart(statsViewModel.weeklyProgress) { item in
                    BarMark(
...
class StatsViewModel: ObservableObject {
    ...
    private func loadAchievements() {
        achievements = [
            Achievement(name: "はじめてのせいかい", icon: "star.fill", color: Color("SuccessColor"), isUnlocked: true),
            Achievement(name: "10もんせいかい", icon: "flame.fill", color: .orange, isUnlocked: totalQuestions >= 10),
            Achievement(name: "れんぞく5かい", icon: "bolt.fill", color: .yellow, isUnlocked: maxStreak >= 5),
            Achievement(name: "レベル3たっせい", icon: "crown.fill", color: .purple, isUnlocked: maxLevel >= 3),
            Achievement(name: "30ぷんプレイ", icon: "clock.fill", color: .blue, isUnlocked: totalPlayTime >= 1800),
            Achievement(name: "パーフェクト", icon: "checkmark.seal.fill", color: .green, isUnlocked: accuracyRate == 100)
        ]
    }
}
